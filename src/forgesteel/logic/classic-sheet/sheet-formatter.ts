import { Ability, AbilitySectionField, AbilitySectionPackage, AbilitySectionRoll, AbilitySectionText } from '@/forgesteel/models/ability';
import { FollowerSheet, ItemSheet, ProjectSheet } from '@/forgesteel/models/classic-sheets/hero-sheet';
import { AbilityLogic } from '@/forgesteel/logic/ability-logic';
...
import { RulesItem } from '@/forgesteel/models/rules-item';
...
export class SheetFormatter {
	...
	static enhanceMarkdown = (text: string) => {
		text = text
			...
			.replace(/\|\s+≤\s*11\s+\|/g, `|![11 or less]()|`)
			.replace(/\|\s+12\s*[-–]\s*16\s+\|/g, `|![12 to 16]()|`)
			.replace(/\|\s+≥?\s*17\s*\+?\s+\|/g, `|![17 or greater]()|`)
			.replace(/11 or lower:?/g, `![11 or less]()`)
			.replace(/12\s*[-–]\s*16:?/g, `![12 to 16]()`)
			.replace(/17\s*\+:?/g, `![17 or greater]()`)
			.replace(/[`]/g, '');
		return text;
	};
